 ...
 */
public class AWS4Signer extends com.datadynamics.bigdata.api.auth.AbstractAWSSigner {
    ...
    private AWSCredentials getCredential(String username, String password) {
    ...
    public DefaultRequest sign(HttpServletRequest servletRequest, String username, String password) throws IOException {
        AWSCredentials credential = getCredential(username, password);

        AmazonWebServiceRequest noop = AmazonWebServiceRequest.NOOP;
        noop.getRequestClientOptions().setReadLimit(servletRequest.getContentLength() + 100000);

        DefaultRequest request = new DefaultRequest(noop, "Amazon S3");
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        IOUtils.copy(servletRequest.getInputStream(), baos);
        request.setContent(new ByteArrayInputStream(baos.toByteArray()));
        request.setEndpoint(URI.create(servletRequest.getRequestURL().toString()));
        request.setHttpMethod(HttpMethodName.valueOf(servletRequest.getMethod().toUpperCase()));
        String[] authorizations = getSignedHeadersString(servletRequest.getHeader("Authorization")).split(";");
    ...
    public void sign(SignableRequest<?> request, AWSCredentials credentials, String authorizationHeader) {
        ...
        // PUT Object의 경우 Content Hash가 매우 중요하다
        ...
        final String canonicalRequest = createCanonicalRequest(request, contentSha256, boto3, authorizationHeader);
        final String stringToSign = createStringToSign(canonicalRequest.trim(), signerParams);
        ...
        request.addHeader(AUTHORIZATION, buildAuthorizationHeader(request, signature, sanitizedCredentials, signerParams, authorizationHeader));

        processRequestPayload(request, signature, signingKey, signerParams);
    }

    public void presignRequest(SignableRequest<?> request, AWSCredentials credentials, Date userSpecifiedExpirationDate, String authorizationHeader) {
        // anonymous credentials, don't sign
        if (isAnonymous(credentials)) {
            return;
        }
    ...
    protected String createCanonicalRequest(SignableRequest<?> request,
        ...
        String path = null;
        if (boto3) {
            // BOTO3의 경우 /가 없이 요청이 들어온다. 따라서 Java와 다르게 /를 삭제해야 한다.
        ...
        canonicalRequestBuilder.append(LINE_SEPARATOR)
                ...
                .append(LINE_SEPARATOR)
                .append(getSignedHeadersString(authorizationHeader)).append(LINE_SEPARATOR)
    ...
    protected String createStringToSign(String canonicalRequest, AWS4SignerRequestParams signerParams) {
        ...
        stringToSignBuilder.append(LINE_SEPARATOR)
                ...
                .append(LINE_SEPARATOR)
                .append(BinaryUtils.toHex(hash(canonicalRequest.trim())));
    ...
    private String buildAuthorizationHeader(SignableRequest<?> request,
                                            ...
                                            AWS4SignerRequestParams signerParams, String authorizationHeader) {
        ...
        final String signerHeaders = "SignedHeaders=" + getSignedHeadersString(authorizationHeader);
    ...
    /**
     * SignedHeaders를 Request에서 추출한다.
     */
    public static String getSignedHeadersString(String authorizationHeader) {
        return org.apache.commons.lang.StringUtils.substringBetween(authorizationHeader, "SignedHeaders=", ", Signature=");
    }
