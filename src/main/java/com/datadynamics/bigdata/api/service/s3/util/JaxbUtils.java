...
public class JaxbUtils {
     ...
     * <p>
     * <pre>
     * Marshaller marshaller = JaxbUtils.createMarshaller("org.opencloudengine.flamingo2.domain.request");
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     * <br/>
     * <b>Marshaller와 Unmarshaller는 재사용이 가능하지만 Thread-Safe하지는 않다.</b>
     *
     * @param packageName 패키지명
     * @return {@link Marshaller} 인스턴스
     * @throws JAXBException Marshaller를 생성할 수 없는 경우(예; 패키지명 오류)
     ...
     * <p>
     * <pre>
     * Unmarshaller unmarshaller = JaxbUtils.createUnmarshaller("org.opencloudengine.flamingo2.domain.request");
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     ...
     *
     * @param packageName 패키지명
     * @return {@link Unmarshaller} 인스턴스
     * @throws JAXBException Unmarshaller를 생성할 수 없는 경우(예; 패키지명 오류)
     * @see "http://www.docjar.com/docs/api/javax/xml/bind/JAXBContext.html"
     */
     ...
     * </p>
     *
     * @param marshaller {@link Marshaller} 인스턴스
     * @param object     XML의 JAXB DOM Object
     * @param filename   저장할 XML 파일명
     * @throws JAXBException         마샬링 할 수 없는 경우
     * @throws FileNotFoundException 해당 파일이 존재하지 않는 경우(예; 디렉토리)
     */
     ...
     * <pre>
     * Request requst = ...;
     * JaxbUtils.marshal(request, "org.opencloudengine.flamingo2.domain.request", "/home/tester/test.xml", "/home/tester/test.xsd");
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     ...
     * <pre>
     * Request requst = ...;
     * JaxbUtils.marshal(request, "org.opencloudengine.flamingo2.domain.request", "/home/tester/test.xml");
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     ...
     * String xml = JaxbUtils.marshal("org.opencloudengine.flamingo2.domain.request", request);
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     *
     * @param packageName 패키지명 (예; "<tt>org.opencloudengine.flamingo2.sal.domain.request</tt>")
     * @param object      XML로 변환하고자 하는 JAXB DOM Object
     * @return String type의 XML데이터
     * @throws JAXBException 마샬링 할 수 없는 경우
     * @throws IOException   XML 파일을 로딩할 수
     */
     ...
     * <p>
     * <pre>
     * Request request = (Request) JaxbUtils.marshal("org.opencloudengine.flamingo2.domain.request", "/home/tester/xml", "test.xml");
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     ...
     * @throws JAXBException unmarshal할 수 없는 경우
     */
     ...
     * Request request = (Request) JaxbUtils.marshal("org.opencloudengine.flamingo2.domain.request", xml);
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     ...
     * Request request = (Request) JaxbUtils.marshal("org.opencloudengine.flamingo2.domain.request", inputStream);
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
     ...
     * Request request = (Request) JaxbUtils.marshal("org.opencloudengine.flamingo2.domain.request", reader);
     * </pre>
     * </p>
     * <b>이 메소드를 반복적으로 호출하면 성능 문제가 발생한다. {@link JAXBContext#createMarshaller()} 및 {@link JAXBContext#createUnmarshaller()} 메소는
     * 반복적으로 호출하는 경우 성능 문제를 발생하기 때문이다.</b>
